...
/**
 * Self-contained animated grid background. Scoped to its own container
 * (not the viewport) so it's safe to use more than once on a page — the
 * upstream version used `fixed inset-0` + `window` listeners, which only
 * works for a single full-viewport instance.
 */
export function KineticGrid({
  ...
  const wrapperClassName = cn(
    ...
    globalColor === "default" ? "bg-[#161618]" : "bg-[#070707]",
    className
  );
